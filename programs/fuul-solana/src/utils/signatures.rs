...
pub fn validate_message_domain(domain: &MessageDomain) -> Result<()> {
    // Validate the program_id matches
    require!(domain.program_id == crate::ID, FuulError::ProgramIdMismatch);

    // Validate the version matches
    require!(domain.version == VERSION, FuulError::VersionMismatch);

    // Validate the deadline hasn't expired
    let clock = Clock::get()?;
    require!(clock.unix_timestamp <= domain.deadline, FuulError::DeadlineExpired);
...
pub fn validate_ed25519_ix(
    ...
    require!(current_ix_index > 0, FuulError::InvalidInstructionSysvar);

    // Load the immediately preceding instruction (the Ed25519 ix)
    let ed_ix = ix_sysvar::load_instruction_at_checked(current_ix_index - 1, ix_sysvar_account)
        .map_err(|_| error!(FuulError::InvalidInstructionSysvar))?;

    // Ensure it is the Ed25519 program and uses no accounts (stateless check)
    require!(ed_ix.program_id == ED25519_PROGRAM_ID, FuulError::BadEd25519Program);
    require!(ed_ix.accounts.is_empty(), FuulError::BadEd25519Accounts);
...
pub fn parse_ed25519_ix_data(data: &[u8]) -> Result<Vec<Ed25519SignatureOffsets>> {
    ...
    require!(data.len() >= MIN_HEADER_LEN + SIG_ENTRY_LEN, FuulError::InvalidInstructionSysvar);

    // First byte: number of signatures
    let sig_count: usize = data[0].into();
    require!(sig_count > 0, FuulError::InvalidInstructionSysvar);
    require!(sig_count <= 255, FuulError::InvalidInstructionSysvar);
    ...
    require!(data.len() >= min_header_len, FuulError::InvalidInstructionSysvar);

    // Helper to read u16 offsets (little-endian)
    let read_u16 = |offset: usize| -> Result<u16> {
        let end = offset + 2;
        let src = data.get(offset..end).ok_or(error!(FuulError::InvalidInstructionSysvar))?;
        let mut arr = [0u8; 2];
    ...
    for i in 0..sig_count {
        ...
        let signature_offset = read_u16(base_offset)?.into();
        let signature_instruction_index = read_u16(base_offset + 2)?;
        let public_key_offset = read_u16(base_offset + 4)?.into();
        let public_key_instruction_index = read_u16(base_offset + 6)?;
        let message_data_offset = read_u16(base_offset + 8)?.into();
        let message_data_size = read_u16(base_offset + 10)?.into();
        ...
        require!(data.len() >= signature_offset + SIG_LEN, FuulError::InvalidInstructionSysvar);
        require!(data.len() >= public_key_offset + PUBKEY_LEN, FuulError::InvalidInstructionSysvar);
        require!(
...
pub fn verify_ed25519_signature(ix_sysvar_account: &AccountInfo) -> Result<(Vec<Pubkey>, Vec<u8>)> {
    ...
    let ed_ix = validate_ed25519_ix(ix_sysvar_account, current_ix_index.into())?;

    // Parse the Ed25519 instruction data (may contain multiple signatures)
    let offsets_vec = parse_ed25519_ix_data(&ed_ix.data)?;
    require!(!offsets_vec.is_empty(), FuulError::InvalidInstructionSysvar);
    ...
    // Shrink to fit - ensure the Vec capacity matches the actual number of unique signers
    signers.shrink_to_fit();
