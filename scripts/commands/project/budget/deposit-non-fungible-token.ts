...
export const depositNonFungibleTokenCommand = new Command('deposit-non-fungible-token')
  .requiredOption(
    '-n, --network <network>',
  ...
  .action(async (options) => {
    ...
    const depositNonFungibleTokenIx = await sdk.depositNonFungibleToken({
      authority: wallet.publicKey,
      projectNonce: new BN(projectNonce),
      tokenMint: new PublicKey(tokenMint),
    });

    const tx = new Transaction().add(...depositNonFungibleTokenIx);
