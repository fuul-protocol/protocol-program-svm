...
export const removeCurrencyTokenCommand = new Command('remove-currency-token')
  .requiredOption(
    '-n, --network <network>',
  ...
  .action(async (options) => {
    ...
    const removeCurrencyTokenIx = await sdk.removeCurrencyToken({
      authority: wallet.publicKey,
      tokenMint: new PublicKey(tokenMint),
    });

    const tx = new Transaction().add(...removeCurrencyTokenIx);
