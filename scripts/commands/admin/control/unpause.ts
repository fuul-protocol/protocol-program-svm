...
export const unpauseProgramCommand = new Command('unpause-program')
  .requiredOption(
    '-n, --network <network>',
  ...
  .action(async (options) => {
    ...
    const unpauseProgramIx = await sdk.unpauseProgram({
      authority: wallet.publicKey,
    });

    const tx = new Transaction().add(...unpauseProgramIx);
